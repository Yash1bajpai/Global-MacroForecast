...
### 2. Manual MOCK_DATA Synchronization ~~(RESOLVED in v1.4.0)~~
- **Status:** **RESOLVED.**
- **Resolution:** Implemented hybrid fetching in `dashboard.js`. The frontend attempts dynamic fetching from the cloud API (`/api/dashboard/{country}`) with a 4-second cold-start timeout, falling back gracefully to static `data/forecasts.json` generated by `export_forecasts.py`.
