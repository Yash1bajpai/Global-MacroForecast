...
df = pd.read_csv(
    os.path.join(FEATURES_DIR, "germany_features.csv"),
    index_col=0,
    parse_dates=True
)
...
print("=" * 55)
print("GERMANY GDP - LightGBM Model")
print("=" * 55)
...
ax.set_title("Japan LightGBM - Feature Importance (Gain)", fontsize=13)
ax.set_xlabel("Importance (Gain)")
plt.tight_layout()
plt.savefig("notebooks/germany_lgbm_importance.png", dpi=150)
plt.close()
...
ax.set_title("Japan GDP Growth - LightGBM Forecast vs Actual", fontsize=13)
ax.set_xlabel("Quarter")
ax.set_ylabel("QoQ GDP Growth (%)")
ax.legend()
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig("notebooks/germany_lgbm_forecast.png", dpi=150)
plt.close()
print("\nPlots saved: notebooks/germany_lgbm_forecast.png, notebooks/germany_lgbm_importance.png")

model_path = os.path.join(MODELS_DIR, "germany_lgbm.pkl")
