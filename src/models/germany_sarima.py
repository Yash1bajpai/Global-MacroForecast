...
df = pd.read_csv(
    os.path.join(DATA_PROCESSED_DIR, "germany_master.csv"),
    index_col=0,
    parse_dates=True
)
...
print("=" * 55)
print("GERMANY GDP Growth - SARIMA Model")
print("=" * 55)
...
print("\nNOTE: Germany GDP (CLVMNACSCAB1GQDE) is a level series.")
print("gdp_growth was computed as log-diff in preprocessing, same as US.")

fig, axes = plt.subplots(1, 2, figsize=(14, 4))
plot_acf(series, lags=16, ax=axes[0], title="ACF - Germany GDP Growth")
plot_pacf(series, lags=16, ax=axes[1], title="PACF - Germany GDP Growth")
plt.tight_layout()
plt.savefig("notebooks/germany_acf_pacf.png", dpi=150)
plt.close()
print("\nACF/PACF plot saved: notebooks/germany_acf_pacf.png")
...
print("\nFitting SARIMA(1,0,1)(0,0,0,4) on Germany...")
...
ax.set_title("Germany GDP Growth - SARIMA(1,0,1) Forecast vs Actual", fontsize=13)
ax.set_xlabel("Quarter")
ax.set_ylabel("QoQ GDP Growth (%)")
ax.legend()
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig("notebooks/germany_sarima_forecast.png", dpi=150)
plt.close()
print("\nForecast plot saved: notebooks/germany_sarima_forecast.png")

model_path = os.path.join(MODELS_DIR, "germany_sarima.pkl")
