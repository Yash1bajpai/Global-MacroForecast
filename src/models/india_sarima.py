...
df = pd.read_csv(
    os.path.join(DATA_PROCESSED_DIR, "india_master.csv"),
    index_col=0,
    parse_dates=True
)
...
print("=" * 55)
print("INDIA GDP Growth - SARIMA Model")
print("=" * 55)
...
print("\nNOTE: India GDP series (NAEXKP01INQ657S) is a QoQ seasonally adjusted")
print("growth rate from OECD QNA, mirrored on FRED. No interpolation needed,")
print("so SARIMA is viable (previously skipped when only annual WB data existed).")

fig, axes = plt.subplots(1, 2, figsize=(14, 4))
plot_acf(series, lags=16, ax=axes[0], title="ACF - India GDP Growth")
plot_pacf(series, lags=16, ax=axes[1], title="PACF - India GDP Growth")
plt.tight_layout()
plt.savefig("notebooks/india_acf_pacf.png", dpi=150)
plt.close()
print("\nACF/PACF plot saved: notebooks/india_acf_pacf.png")
...
print("\nFitting SARIMA(1,0,1)(0,0,0,4) on India...")
...
print("\n" + "=" * 55)
print("TEST SET PERFORMANCE (2020-Q1 to latest)")
print("=" * 55)
...
ax.set_title("India GDP Growth - SARIMA(1,0,1) Forecast vs Actual", fontsize=13)
ax.set_xlabel("Quarter")
ax.set_ylabel("QoQ GDP Growth (%)")
ax.legend()
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig("notebooks/india_sarima_forecast.png", dpi=150)
plt.close()
print("\nForecast plot saved: notebooks/india_sarima_forecast.png")

model_path = os.path.join(MODELS_DIR, "india_sarima.pkl")
joblib.dump(fitted_model, model_path)
print(f"Model saved       : {model_path}")

print("\n" + "=" * 55)
print("SARIMA COEFFICIENTS")
print("=" * 55)
