...
df = pd.read_csv(
    os.path.join(FEATURES_DIR, "japan_features.csv"),
    index_col=0,
    parse_dates=True
)
...
print("=" * 55)
print("JAPAN GDP - LightGBM Model")
print("=" * 55)
...
plt.tight_layout()
plt.savefig("notebooks/japan_lgbm_importance.png", dpi=150)
plt.close()
...
plt.tight_layout()
plt.savefig("notebooks/japan_lgbm_forecast.png", dpi=150)
plt.close()
print("\nPlots saved: notebooks/japan_lgbm_forecast.png, notebooks/japan_lgbm_importance.png")

model_path = os.path.join(MODELS_DIR, "japan_lgbm.pkl")
