...
df = pd.read_csv(
    os.path.join(DATA_PROCESSED_DIR, "japan_master.csv"),
    index_col=0,
    parse_dates=True
)
...
print("=" * 55)
print("JAPAN GDP Growth - SARIMA Model")
print("=" * 55)
...
print("\nNOTE: Japan GDP series (NAEXKP01JPQ657S) is already a QoQ growth rate.")
print("It was NOT log-differenced in preprocessing - it came pre-computed from FRED/OECD.")

fig, axes = plt.subplots(1, 2, figsize=(14, 4))
plot_acf(series, lags=16, ax=axes[0], title="ACF - Japan GDP Growth")
plot_pacf(series, lags=16, ax=axes[1], title="PACF - Japan GDP Growth")
plt.tight_layout()
plt.savefig("notebooks/japan_acf_pacf.png", dpi=150)
plt.close()
print("\nACF/PACF plot saved: notebooks/japan_acf_pacf.png")
...
print("\nFitting SARIMA(1,0,1)(0,0,0,4) on Japan...")
...
ax.set_title("Japan GDP Growth - SARIMA(1,0,1) Forecast vs Actual", fontsize=13)
ax.set_xlabel("Quarter")
ax.set_ylabel("QoQ GDP Growth (%)")
ax.legend()
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig("notebooks/japan_sarima_forecast.png", dpi=150)
plt.close()
print("\nForecast plot saved: notebooks/japan_sarima_forecast.png")

model_path = os.path.join(MODELS_DIR, "japan_sarima.pkl")
