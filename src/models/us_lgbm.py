...
df = pd.read_csv(
    os.path.join(FEATURES_DIR, "us_features.csv"),
    index_col=0,
    parse_dates=True
)
...
ax.set_title("US LightGBM - Feature Importance (Gain)", fontsize=13)
ax.set_xlabel("Importance (Gain)")
plt.tight_layout()
plt.savefig("notebooks/us_lgbm_importance.png", dpi=150)
plt.close()
...
ax.set_title("US GDP Growth - LightGBM Forecast vs Actual", fontsize=13)
ax.set_xlabel("Quarter")
ax.set_ylabel("QoQ GDP Growth (%)")
ax.legend()
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig("notebooks/us_lgbm_forecast.png", dpi=150)
plt.close()
print("\nPlots saved: notebooks/us_lgbm_forecast.png, notebooks/us_lgbm_importance.png")

model_path = os.path.join(MODELS_DIR, "us_lgbm.pkl")
